...
def load_data(original_symbol, start, end):
    ...
    for attempt in range(max_retries):
        try:
            # تحميل فقط الأعمدة المطلوبة لتسريع العملية
...
def train_model(df):
    ...
    try:
        model = xgb.XGBClassifier(
            n_estimators=50,        # تقليل عدد الأشجار لتسريع التدريب
            max_depth=3,            # عمق أقل لتسريع
            learning_rate=0.1,
...
# ===== واجهة المستخدم =====
st.title("📈 AI Smart Trader — النسخة السريعة 💜")
