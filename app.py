# app.py — AI Smart Trader Pro (نسخة مستقرة ومصححة)
...
def load_enhanced_data(symbol, start, end):
    ...
    # Volume ratio بطريقة آمنة
    df["Volume_MA20"] = volume.rolling(20).mean()
    df = df.dropna(subset=["Volume_MA20"])  # حذف أي صفوف بها NaN
    df["Volume_Ratio"] = volume.loc[df.index]/df["Volume_MA20"]
...
def prepare_features_for_ml(df, lookback_days):
    ...
    return df[feature_cols], df["Binary_Target"], feature_cols, df
...
def train_advanced_model(X,y,model_type,test_size_ratio):
    ...
    if model_type=="XGBoost":
        model=xgb.XGBClassifier(n_estimators=200,max_depth=6,learning_rate=0.05,subsample=0.8,colsample_bytree=0.8,
...
def generate_trading_signal(model,X_last_row,df_row,conf_threshold):
    try:
        ...
        if pred==1 and proba*100>=conf_threshold:
            signal="شراء قوي 🟢" if rsi<70 and macd>0 and volume_ratio>1 else "شراء 🟢"
        elif pred==0 and (1-proba)*100>=conf_threshold:
            signal="بيع قوي 🔴" if rsi>30 and macd<0 and volume_ratio>1 else "بيع 🔴"
        else: signal="محايد ⚪"
        confidence=max(proba,1-proba)
        return signal,confidence
    except: return "خطأ في الإشارة",0.0
...
def calculate_risk_metrics(df):
    ...
    else: metrics["معدل شارب التقريبي"]="N/A"
    return metrics

# ================= واجهة التطبيق =================
st.title("🎯 AI Smart Trader Pro — النسخة المصححة")
st.markdown("نظام متقدم للتحليل والتنبؤ المالي باستخدام الذكاء الاصطناعي.")
st.info("💡 **مهم:** هذا التطبيق تعليمي فقط. الأسواق المالية خطيرة ولا يوجد أي نموذج يضمن الربح.")
        ...
        if len(X)<100: st.error("❌ البيانات بعد المعالجة غير كافية.")
            ...
            if model is None: st.error(f"❌ فشل في تدريب النموذج: {status}")
                ...
                with tab2:
                    ...
                    if not risk: st.info("لا يمكن حساب مقاييس المخاطرة حالياً.")
                    else: 
                        for k,v in risk.items(): st.metric(k,v)
                with tab3:
                ...
                st.markdown("---")
                st.subheader("💡 توصيات تحليلية (تعليمية فقط)")
                tips=[]
                if rsi_val<30: tips.append("RSI أقل من 30 → الأصل في منطقة تشبع بيع، قد توجد فرصة ارتداد.")
                elif rsi_val>70: tips.append("RSI أعلى من 70 → الأصل في منطقة تشبع شراء، الحذر من التصحيح.")
                if macd_val>0: tips.append("MACD فوق الصفر → الزخم العام يميل للصعود.")
                else: tips.append("MACD تحت الصفر → الزخم العام يميل للهبوط.")
                if vol_ratio>1.5: tips.append("حجم التداول أعلى من المتوسط → حركة السعر الحالية مدعومة بحجم قوي.")
                if len(tips)==0: st.write("• لا توجد إشارات قوية حالياً، الوضع أقرب للحياد.")
                else: 
                    for t in tips: st.write("• "+t)
st.markdown("---")
st.caption("⚠️ كل ما في هذا التطبيق تعليمي فقط، وليس توصية استثمارية حقيقية.")