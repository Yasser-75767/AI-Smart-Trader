# app.py — AI Smart Trader Pro (نسخة مستقرة للهاتف و Streamlit Cloud)
...
# ================= إعداد الصفحة =================
st.set_page_config(page_title="AI Smart Trader Pro — النسخة السريعة المستقرة", layout="wide")
...
model_type = st.sidebar.selectbox(
    "نموذج التعلم الآلي:",
    ["Random Forest", "Gradient Boosting", "Ensemble (RF + GB)"]
)
...
def generate_signal(model, X_last, last_row, conf_threshold):
    ...
    # إشارة أساسية من النموذج + تصفية بالمؤشرات
...
st.title("🎯 AI Smart Trader Pro — نسخة مستقرة وسريعة")
st.info("💡 أداة تعليمية فقط. لا تعتبر هذه النتائج توصية استثمارية حقيقية.")
...
if st.button("🚀 بدء التحليل"):
    with st.spinner("جاري تحميل البيانات وتحليلها..."):
        ...
        if df.empty:
            st.error("❌ لا توجد بيانات كافية لهذا الأصل في هذه الفترة (تحتاج ~150 شمعة على الأقل).")
        else:
            ...
            if model is None:
                ...
                tab1, tab2, tab3, tab4 = st.tabs(
                    ["المؤشرات الفنية", "مقاييس المخاطرة", "تحليل المشاعر", "البيانات التاريخية"]
                )
                ...
                # توصيات نصّية بسيطة
                st.markdown("---")
                st.subheader("💡 توصيات تعليمية")
                tips = []
                if rsi_val < 30:
                    tips.append("RSI أقل من 30 → تشبع بيع، قد توجد فرصة ارتداد.")
                elif rsi_val > 70:
                    tips.append("RSI أعلى من 70 → تشبع شراء، الحذر من التصحيح.")

                if macd_val > 0:
                    tips.append("MACD موجب → الزخم يميل للصعود.")
                else:
                    tips.append("MACD سالب → الزخم يميل للهبوط.")

                if vol_ratio > 1.5:
                    tips.append("حجم التداول أعلى من المتوسط → الحركة الحالية قوية.")

                if not tips:
                    st.write("• لا توجد إشارات قوية، الوضع أقرب للحياد.")
                else:
                    for t in tips:
                        st.write("• " + t)

st.markdown("---")
st.caption("⚠️ هذا التطبيق تعليمي فقط، لا يُستخدم لاتخاذ قرارات تداول حقيقية.")