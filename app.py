# app.py — AI Smart Trader Pro (نسخة مُصححة وجاهزة)
...
# ================= إعداد الصفحة =================
st.set_page_config(page_title="AI Smart Trader Pro — النسخة المُصححة", layout="wide")
...
def load_enhanced_data(symbol, start, end):
    ...
    # التأكد من أن الأعمدة Series أحادية البعد
    close = df["Close"]
...
def prepare_features_for_ml(df, lookback_days):
    base_features = [
        "Close","SMA_5","SMA_20","SMA_50","EMA_10","EMA_50","RSI_14",
...
def generate_trading_signal(model,X_last_row,df_row,conf_threshold):
    try:
        ...
        if pred==1 and proba*100>=conf_threshold:
            if rsi<70 and macd>0 and vol_ratio>1:
                signal="شراء قوي 🟢"
            else: signal="شراء 🟢"
        elif pred==0 and (1-proba)*100>=conf_threshold:
            if rsi>30 and macd<0 and vol_ratio>1:
                signal="بيع قوي 🔴"
            else: signal="بيع 🔴"
        else:
            signal="محايد ⚪"
...
# ================= واجهة التطبيق =================
st.title("🎯 AI Smart Trader Pro — النسخة المُصححة")
...
if not st.session_state.get("run",False):
    ...
    if df.empty:
        st.error("❌ لا توجد بيانات كافية لهذا الأصل في هذه الفترة.")
    else:
...
st.markdown("---")
st.caption("⚠️ كل ما في هذا التطبيق تعليمي + عملي، يمكن استخدامه للتداول العملي، احذر المخاطر.")