# app.py — AI Smart Trader Pro (نسخة مستقرة ومحسنة)
...
def prepare_features_for_ml(df, lookback_days):
    ...
    available_features = [f for f in base_features if f in df.columns and pd.api.types.is_numeric_dtype(df[f])]
...
def calculate_risk_metrics(df):
    ...
    metrics["معدل شارب التقريبي"] = f"{returns.mean()/returns.std()*np.sqrt(252):.2f}" if returns.std()>0 else "N/A"
    return metrics
    ...
    if df.empty:
        ...
        if len(X)<100:
            ...
            if model is None:
                ...
                tips=[]
                if rsi_val<30:
                    tips.append("RSI أقل من 30 → الأصل في منطقة تشبع بيع، قد توجد فرصة ارتداد.")
                elif rsi_val>70:
                    tips.append("RSI أعلى من 70 → الأصل في منطقة تشبع شراء، الحذر من التصحيح.")
                if macd_val>0:
                    tips.append("MACD فوق الصفر → الزخم العام يميل للصعود.")
                else:
                    tips.append("MACD تحت الصفر → الزخم العام يميل للهبوط.")
                if vol_ratio>1.5:
                    tips.append("حجم التداول أعلى من المتوسط → حركة السعر الحالية مدعومة بحجم قوي.")
                if len(tips)==0:
                    st.write("• لا توجد إشارات قوية حالياً، الوضع أقرب للحياد.")
                else:
                    for t in tips:
                        st.write("• "+t)
st.markdown("---")
