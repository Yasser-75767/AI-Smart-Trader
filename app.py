# app_final.py — AI Smart Trader Pro (نسخة نهائية ومحسّنة)
...
def load_enhanced_data(symbol, start, end):
    ...
    volume_aligned, sma_aligned = volume.align(df["Volume_SMA"], join='left')
    df["Volume_Ratio"] = (volume_aligned / sma_aligned.replace(0,np.nan)).fillna(1.0)

    df = df.dropna()
    return df

def create_advanced_target(df, lookahead=1):
    df["Binary_Target"] = (df["Close"].shift(-lookahead) > df["Close"]).astype(int)
    return df.dropna()

def prepare_features_for_ml(df, lookback_days):
    base_features = ["Close", "SMA_5", "SMA_20", "SMA_50", "EMA_10", "EMA_50", "RSI_14", "MACD", "MACD_Signal", "MACD_Hist",
                     "BB_Upper","BB_Lower","BB_Width","Stoch_K","Stoch_D","Williams_R","CCI","ADX","OBV","ATR",
                     "Returns","Log_Returns","Price_Range","Gap","Volatility_5D","Volatility_21D","Momentum_5D","Momentum_21D",
...
def train_advanced_model(X, y, model_type, test_size_ratio):
    ...
    if model_type=="XGBoost":
        ...
        model = VotingClassifier([("xgb", xgb.XGBClassifier(n_estimators=50,use_label_encoder=False,eval_metric="logloss")),
                                  ("rf", RandomForestClassifier(n_estimators=50,random_state=42)),
                                  ("gb", GradientBoostingClassifier(n_estimators=50,random_state=42))], voting="soft")
    model.fit(X_train, y_train)
    y_pred = model.predict(X_test)
    acc = accuracy_score(y_test, y_pred)
    return model, acc

def generate_trading_signal(model, X_last, df_row, conf_threshold):
    try:
...
if not st.session_state.get("run", False):
    st.warning("اضغط على زر 🚀 بدء التحليل لبدء العمل.")
else:
